...
void MostrarD(char[][C],int[],int,int);
void OrdenarV(int[],int[],int);
int main(){
    char mButacas[F][C];
...
void MostrarD(char m[][C],int v[],int filas,int col){
    int i,j,vCont[col],posBut;
    ...
    printf("\n\nBUTACAS   CANTIDAD");
    OrdenarV(vCont,v,col);
    for(j=0;j<col;j++){
        printf("\n%3d %7d",v[j],vCont[j]);
    }
}

void OrdenarV(int vCont[],int vBut[],int ce){
    int auxCont,auxBut,j,cota=ce-1,desordenado=1;
